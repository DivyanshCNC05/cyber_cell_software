...
function validate_pdf_upload(array $file): array {
  ...
  // size limit (change if needed)
  $maxBytes = 5 * 1024 * 1024; // 5 MB
  if (($file['size'] ?? 0) > $maxBytes) {
    return [false, 'PDF must be <= 5MB'];
  }

  // MIME check using finfo (don’t trust browser content-type) [web:385]
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!isset($CEIR_TABLES[$selected])) {
    ...
    if ($name === '' || $date_of_complaint === '' || $imei === '' || $lost_found === '' || $block_unblock === '') {
      ...
      // PDF optional (if you want mandatory, enforce here)
      $pdfPathDb = null;
      ...
      if (!empty($_FILES['pdf']['name'])) {
        ...
        if (!$ok) {
          ...
          $uploadDirFs = __DIR__ . '/../uploads/ceir_pdfs/';   // filesystem path
          $uploadDirDb = '/uploads/ceir_pdfs/';                // browser path
...
<div class="container py-4">
  ...
  <form method="post" enctype="multipart/form-data" class="row g-3">
    ...
    <div class="col-md-4">
      ...
      <input type="date" class="form-control" name="date_of_complaint" required value="<?= htmlspecialchars($_POST['date_of_complaint'] ?? '') ?>">
    </div>
    ...
    <div class="col-md-4">
      ...
      <input class="form-control" name="imei" required value="<?= htmlspecialchars($_POST['imei'] ?? '') ?>">
    </div>
    ...
    <div class="col-md-4">
      ...
      <div class="form-text">Only PDF, max 5MB.</div>
    </div>
